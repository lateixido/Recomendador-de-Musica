...
# --- 2) Preparar features ---
# Extraemos las features numéricas para el modelo
num_features = df[["Danceability", "Energy", "Positiveness", "Loudness"]].to_numpy(dtype=float)

# Escalamos las features numéricas con StandardScaler
...
# Comentario sobre hiperparámetros:
# - n_neighbors = 10 → cantidad de recomendaciones
# - metric = cosine → similaridad angular entre vectores
# - algorithm = brute → recomendado con pocas features y métrica cosine

# --- 4) Construir vector de features para una canción ---
...
# --- 5) Buscar índice de una canción ---
...
# --- 6) Función principal de recomendación ---
def recommend_by_track_name(track: str, top_k: int = 10, artist: str | None = None):
    ...
    for d, i in zip(distances[0], indices[0]):
        if i == idx:  # saltamos la canción semilla
            continue
        recs.append({
            "song": df.loc[i, "song"],
